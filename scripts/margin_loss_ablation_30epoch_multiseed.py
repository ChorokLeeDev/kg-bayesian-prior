#!/usr/bin/env python3
"""
Margin Loss Ablation Study for CAGP - 30 Epochs, Multi-Seed Version

Extended version of margin_loss_ablation.py with:
- 30 epochs (vs 8)
- 3 seeds (42, 123, 456)
- Both WN18RR and FB15k-237

Output:
    outputs/margin_loss_ablation_30epoch_results.json

Saves per-seed results with mean and std aggregation.
"""
...
def train_model(model, triples, device, epochs=30, unc_weight=0.1, batch_size=512, seed=42):
...
def run_ablation_study(dataset_name, train_ds, test_ds, device, seed):
    """Run full ablation on a single dataset with given seed."""
    ...
    # CoverageOnly
    print("  [1/4] CoverageOnly...", end=" ", flush=True)
    m_cov = CoverageOnly(n_ent, n_rel)
    m_cov.precompute_coverage(train_triples)
    m_cov = train_model(m_cov, train_triples, device, epochs=30, unc_weight=0.0, seed=seed)
    ...
    print(f"Overall: {r_cov_overall:.4f}")
    del m_cov; gc.collect()
    
    # GPOnly
    print("  [2/4] GPOnly...", end=" ", flush=True)
    m_gp = GPOnly(n_ent, n_rel)
    m_gp.precompute_coverage(train_triples)
    m_gp = train_model(m_gp, train_triples, device, epochs=30, unc_weight=0.0, seed=seed)
    ...
    print(f"Overall: {r_gp_overall:.4f}")
    del m_gp; gc.collect()
    
    # CAGP with margin loss
    print("  [3/4] CAGP (w_unc=0.1)...", end=" ", flush=True)
    m_with = CAGP(n_ent, n_rel)
    m_with.precompute_coverage(train_triples)
    m_with = train_model(m_with, train_triples, device, epochs=30, unc_weight=0.1, seed=seed)
    ...
    print(f"Overall: {r_with_overall:.4f}, Alpha: {alpha_with:.4f}")
    del m_with; gc.collect()
    
    # CAGP without margin loss (ABLATION)
    print("  [4/4] CAGP (w_unc=0.0)...", end=" ", flush=True)
    m_without = CAGP(n_ent, n_rel)
    m_without.precompute_coverage(train_triples)
    m_without = train_model(m_without, train_triples, device, epochs=30, unc_weight=0.0, seed=seed)
    ...
    print(f"Overall: {r_without_overall:.4f}, Alpha: {alpha_without:.4f}")
...
def aggregate_results(all_seeds_results):
    """Aggregate per-seed results to mean and std."""
    aggregated = {}
    
    for method in all_seeds_results[0].keys():
        overall_scores = [seed_result[method]['overall'] for seed_result in all_seeds_results]
        emerging_scores = [seed_result[method]['emerging'] for seed_result in all_seeds_results 
                          if seed_result[method]['emerging'] is not None]
        
        aggregated[method] = {
            'overall_mean': float(np.mean(overall_scores)),
            'overall_std': float(np.std(overall_scores)),
            'overall_scores': [float(x) for x in overall_scores],
        }
        
        if emerging_scores:
            aggregated[method]['emerging_mean'] = float(np.mean(emerging_scores))
            aggregated[method]['emerging_std'] = float(np.std(emerging_scores))
            aggregated[method]['emerging_scores'] = [float(x) for x in emerging_scores]
...
def main():
    ...
    print("\n" + "=" * 70)
    print("MARGIN LOSS ABLATION - 30 EPOCHS, 3 SEEDS")
    print("=" * 70)
    print("Tests whether CAGP performance depends on the uncertainty margin loss")
    print("with extended training (30 epochs) and multiple seeds.\n")
    ...
    print(f"Entities: {train_ds_wn.num_entities}, Relations: {train_ds_wn.num_relations}")
    print(f"Train: {len(train_ds_wn.triples)}, Test: {len(test_ds_wn.triples)}")
    
    wn_per_seed = []
    for seed in seeds:
        result = run_ablation_study('WN18RR', train_ds_wn, test_ds_wn, device, seed)
        wn_per_seed.append(result)
    
    all_results['WN18RR'] = {
        'per_seed': wn_per_seed,
        'aggregated': aggregate_results(wn_per_seed)
    }
    ...
    try:
        ...
        print(f"Entities: {train_ds_fb.num_entities}, Relations: {train_ds_fb.num_relations}")
        print(f"Train: {len(train_ds_fb.triples)}, Test: {len(test_ds_fb.triples)}")
        
        fb_per_seed = []
        for seed in seeds:
            result = run_ablation_study('FB15k-237', train_ds_fb, test_ds_fb, device, seed)
            fb_per_seed.append(result)
        
        all_results['FB15k-237'] = {
            'per_seed': fb_per_seed,
            'aggregated': aggregate_results(fb_per_seed)
        }
    ...
    output_path = output_dir / 'margin_loss_ablation_30epoch_results.json'
    ...
    # Print summary
    print(f"\n{'='*70}")
    print("SUMMARY (30 epochs, 3 seeds)")
    print(f"{'='*70}")
    
    for dataset_name in ['WN18RR', 'FB15k-237']:
        if dataset_name not in all_results:
            continue
        
        print(f"\n{dataset_name}:")
        print("-" * 70)
        agg = all_results[dataset_name]['aggregated']
        
        print(f"{'Method':<25} {'Overall (Mean±Std)':<25} {'Emerging (Mean±Std)':<25}")
        print("-" * 70)
        
        for method in ['CoverageOnly', 'GPOnly', 'CAGP_with_margin', 'CAGP_no_margin']:
            if method not in agg:
                continue
            overall_str = f"{agg[method]['overall_mean']:.4f}±{agg[method]['overall_std']:.4f}"
            emerging_str = "N/A"
            if 'emerging_mean' in agg[method]:
                emerging_str = f"{agg[method]['emerging_mean']:.4f}±{agg[method]['emerging_std']:.4f}"
            print(f"{method:<25} {overall_str:<25} {emerging_str:<25}")
        
        print("\nKEY FINDINGS:")
        with_margin = agg['CAGP_with_margin']['overall_mean']
        without_margin = agg['CAGP_no_margin']['overall_mean']
        gp_only = agg['GPOnly']['overall_mean']
        cov_only = agg['CoverageOnly']['overall_mean']
        
        margin_contrib = with_margin - without_margin
        diff_to_gp = without_margin - gp_only
        diff_to_cov = without_margin - cov_only
        
        print(f"  • Margin loss contribution: {margin_contrib:+.4f}")
        print(f"  • CAGP (w_unc=0.0) vs GPOnly: {diff_to_gp:+.4f} (beats: {without_margin > gp_only})")
        print(f"  • CAGP (w_unc=0.0) vs CoverageOnly: {diff_to_cov:+.4f} (beats: {without_margin > cov_only})")
