#!/usr/bin/env python3
"""
Run MC Dropout, Deep Ensemble, and SNGP baselines on ICEWS18 temporal OOD.

ICEWS18 uses ground-truth timestamps (not simulated coverage splits).
...
from src.data.loaders import load_icews18
...
def main():
    ...
    train_ds, _, test_ds = load_icews18()
    train = train_ds.triples
    test = test_ds.triples
    n_ent = train_ds.num_entities
    n_rel = train_ds.num_relations
    print(f"ICEWS18: {n_ent} entities, {n_rel} relations, {len(train)} train, {len(test)} test")
    ...
    # Summary
    print("\n" + "=" * 60)
    print("ICEWS18 SUMMARY (mean ± std over 3 seeds)")
    print("=" * 60)
    ...
    # Save
    out = project_root / 'outputs' / 'icews18_missing_baselines.json'
