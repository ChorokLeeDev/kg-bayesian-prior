...
\textbf{Uncertainty in KGE.} UKGE~\citep{chen2019ukge} models confidence scores per triple, while BEUrRE~\citep{chen2021beurre} uses box embeddings with learned uncertainty bounds. Calibration analysis~\citep{safavi2020evaluating} has shown KGE models are poorly calibrated. These methods focus on semantic uncertainty derived from embeddings---our work shows structural uncertainty is equally important but orthogonal, and that embedding-based methods systematically miss it.

\textbf{Bayesian Deep Learning.} MC Dropout~\citep{gal2016dropout} approximates Bayesian inference via dropout at test time, while Deep Ensembles~\citep{lakshminarayanan2017simple} capture uncertainty through model disagreement. Energy-based OOD detection~\citep{liu2020energy} uses score magnitude. The epistemic-aleatoric decomposition~\citep{kendall2017uncertainties} distinguishes model from data uncertainty. We show these methods fail specifically on the coverage blind spot because they measure parameter uncertainty, not data coverage.

\textbf{OOD Detection.} Generalized OOD detection~\citep{yang2021generalized} encompasses various distribution shift types. Prior KG work focuses on novel entities or temporal shift. We identify a distinct failure mode---novel entity-relation \emph{contexts}---invisible to existing embedding-based methods.
...
\begin{table}[h]
\centering
\caption{MC Dropout AUROC vs. number of passes for novel-context detection. Increasing passes does not improve detection, confirming the issue is architectural.}
\label{tab:mc_ablation}
\begin{tabular}{lcccc}
\toprule
Dataset & 10 passes & 20 passes & 30 passes & 50 passes \\
\midrule
FB15k-237 & .60 & .61 & .60 & .61 \\
WN18RR & .47 & .47 & .48 & .47 \\
\bottomrule
\end{tabular}
\end{table}
