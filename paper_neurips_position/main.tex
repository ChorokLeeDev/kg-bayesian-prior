...
\textbf{Uncertainty in KGE.} UKGE~\citep{chen2019ukge} models confidence scores per triple, while BEUrRE~\citep{chen2021beurre} uses box embeddings with learned uncertainty bounds. Calibration analysis~\citep{safavi2020evaluating,zhu2024reliablekg} has shown KGE models are poorly calibrated. Recent work proposes conformal prediction for KGE uncertainty~\citep{zhu2025certainty,zhu2025predicate}, providing statistical guarantees but still deriving uncertainty from embeddings. Surveys~\citep{munikoti2024challenges} identify uncertainty quantification as a key challenge. These methods focus on semantic uncertainty derived from embeddings---our work shows structural uncertainty is equally important but orthogonal, and that embedding-based methods systematically miss it.
