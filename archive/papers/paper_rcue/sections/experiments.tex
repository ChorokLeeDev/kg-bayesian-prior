...
\midrule
Energy & 0.62{\tiny $\pm$.01} & 0.78{\tiny $\pm$.00} & 0.46 & 0.89 \\
...
\midrule
\textbf{RCUE} & \textbf{0.96}{\tiny $\pm$.02} & \textbf{0.83}{\tiny $\pm$.16} & \textbf{0.92} & \textbf{0.999} \\
\bottomrule
\end{tabular}
\end{table}

$^*$YAGO3-10 Energy baseline shows 0.46 AUROC, lower than random due to inverse correlation between confidence and novel contexts.
...
\begin{itemize}
    ...
    \item RCUE achieves near-perfect detection on ICEWS14 (0.999)
    \item Improvement over Energy ranges from +5pp (WN18RR) to +46pp (YAGO3-10)
\end{itemize}
...
To address whether RCUE provides value beyond a binary lookup table, we compare Expected Calibration Error (ECE) and uncertainty spread.

\begin{table}[h]
\centering
\caption{Calibration comparison on FB15k-237. Coverage-Only has zero spread (binary), while RCUE provides meaningful gradations. Lower ECE is better.}
\label{tab:ece}
\begin{tabular}{lcccc}
\toprule
Method & ECE (All) & ECE (ID) & ID std & OOD std \\
\midrule
Energy & 0.42 & 0.44 & 1.14 & 1.50 \\
Coverage-Only & 0.31 & 0.45 & \textbf{0.00} & \textbf{0.00} \\
\textbf{RCUE} & \textbf{0.31} & \textbf{0.41} & 0.05 & 0.30 \\
\bottomrule
\end{tabular}
\end{table}
...
Key findings:
\begin{itemize}
    \item \textbf{Coverage-Only has zero variance} within both ID and OOD classes---all samples receive identical uncertainty (0 or 1)
    \item \textbf{RCUE provides gradation}: uncertainty varies meaningfully within both classes (ID std = 0.05, OOD std = 0.30)
    \item RCUE matches Coverage-Only's overall ECE (0.31) while providing \emph{calibrated} rather than binary uncertainty
    \item Within the ID class, RCUE achieves better ECE (0.41 vs 0.45) because the MLP learns semantic uncertainty
\end{itemize}

This demonstrates that RCUE is not merely a lookup table. While Coverage-Only achieves perfect OOD \emph{detection}, RCUE provides calibrated uncertainty \emph{estimation} that distinguishes ``slightly OOD'' from ``completely OOD'' samples.
