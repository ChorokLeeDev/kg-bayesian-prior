...
\begin{itemize}
    ...
    \item MC Dropout's ability to detect these failures: \textbf{0.61 AUROC} (near random)
    \item Deep Ensemble's ability: \textbf{0.59 AUROC} (near random)
    \item A simple coverage lookup: \textbf{perfect detection} (1.00 AUROC) of zero-coverage queries
\end{itemize}

The practical implication is immediate: \textbf{embedding-based uncertainty methods miss the structural blind spot entirely}, while a trivial lookup catches it perfectly.
...
\begin{enumerate}
    ...
    \item \textbf{Evidence}: We demonstrate that MC Dropout and Deep Ensembles perform near random on the structural blind spot, while coverage achieves perfect detection, across 4 datasets, 4 architectures, and with statistical significance (Section~\ref{sec:evidence}).
...
\subsection{Bayesian Methods Perform Near Random}

Table~\ref{tab:auroc} shows the critical finding: \textbf{MC Dropout and Deep Ensemble perform near random} (AUROC $\approx 0.5$--$0.6$) on detecting the coverage blind spot, while Coverage achieves near-perfect detection.

\begin{table}[htbp]
\centering
\caption{Uncertainty method comparison (AUROC). MC Dropout and Deep Ensemble perform near random on novel-context detection, while Coverage achieves near-perfect detection. Simply combining Energy with Coverage (last column) improves reliability.}
\label{tab:auroc}
...
\midrule
\multirow{4}{*}{Temporal}
& FB15k-237 & .626 & .606 & .512 & .935 & \textbf{.935} \\
& WN18RR & .520 & .577 & .600 & .859 & \textbf{.859} \\
& YAGO3-10 & .352 & .606 & .722 & .840 & \textbf{.840} \\
& ICEWS14 & .732 & .799 & .590 & .993 & \textbf{.993} \\
\midrule
\multirow{4}{*}{Novel-Ctx}
& FB15k-237 & .606 & .588 & .509 & 1.00 & \textbf{1.00} \\
& WN18RR & .513 & .596 & .627 & 1.00 & \textbf{1.00} \\
& YAGO3-10 & .482 & .712 & .623 & 1.00 & \textbf{1.00} \\
& ICEWS14 & .760 & .823 & .589 & 1.00 & \textbf{1.00} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Why do Bayesian methods fail to reliably detect novel contexts?} They measure semantic uncertainty---embedding variance---which is \emph{uncorrelated} with structural uncertainty. An entity can have well-trained embeddings (low dropout variance) while being completely novel in a particular relational context (high structural uncertainty).
...
\begin{enumerate}
    ...
    \item \textbf{Do not rely on MC Dropout or Deep Ensembles alone.} These methods perform near random on the coverage blind spot. If you use them, combine with structural coverage.
...
We have argued that \textbf{standard uncertainty quantification methods for knowledge graph embeddings should not be trusted in isolation}. MC Dropout and Deep Ensembles capture semantic uncertainty (embedding confidence) but miss structural uncertainty (context coverage). On the coverage blind spot, these methods perform only \emph{near random}, while a simple coverage lookup achieves perfect detection.

The fix is simple: track entity-relation coverage with a hash table. This trivial lookup catches failures that sophisticated Bayesian methods miss entirely.
...
\begin{table}[h]
\centering
\caption{Bayesian KGE baselines. UKGE variants and SNGP fail to detect novel-context queries, confirming that embedding-derived uncertainty cannot capture structural gaps.}
...
\midrule
\multirow{4}{*}{FB15k-237}
& UKGE-Logi & .58 & .79 & \textbf{.50} \\
& UKGE-Rect & .39 & .52 & \textbf{.33} \\
& UKGE-PSL & .67 & .84 & \textbf{.60} \\
& SNGP & .46 & .62 & \textbf{.39} \\
\midrule
\multirow{2}{*}{YAGO3-10}
& UKGE & .68 & .79 & \textbf{.58} \\
